...
class Assignment(assignment.Driver):
    ...
    # sidinfo crud
    @sql.handle_conflicts(conflict_type='sidinfo')
    def create_sidinfo(self, sidinfo):
        with sql.transaction() as session:
            ref = Sidinfo.from_dict(sidinfo)
            session.add(ref)
        return ref.to_dict()

    def get_sidinfo(self, sid_id):
        with sql.transaction() as session:
            sidinfo_ref = session.query(Sidinfo).get(sid_id)
            print("assignment/backends/sql.py: sidinfo = ", sidinfo_ref)
            if sidinfo_ref is None:
		return None
        return sidinfo_ref.to_dict()

    def _get_sidinfo(self, session, sid_id):
        sid_ref = session.query(Sidinfo).get(sid_id)
        if sid_ref is None:
            raise exception.ProjectNotFound(sid_id=sid_id)
        return sid_ref

    def delete_sidinfo(self, sid_id):
        with sql.transaction() as session:
            ref = self._get_sidinfo(session, sid_id)
            session.delete(ref)
...
class Sidinfo(sql.ModelBase, sql.DictBase):
    __tablename__ = 'sidinfo'
    attributes = ['id', 'members']
    id = sql.Column(sql.String(64), primary_key=True)
    members = sql.Column(sql.JsonBlob)
    extra = sql.Column(sql.JsonBlob())
