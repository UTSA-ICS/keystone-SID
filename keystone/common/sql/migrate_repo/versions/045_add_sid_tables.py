...
def upgrade(migrate_engine):
    ...
    sid_table.create(migrate_engine, checkfirst=True)

    sql.Table('user', meta, autoload=True)
    user_sid_metadata_table = sql.Table(
        'user_sid_metadata',
        meta,
        sql.Column(
            'user_id',
        ...
        sql.Column(
            'sid_id',
            sql.String(64),
            sql.ForeignKey('sid.id'),
            primary_key=True),
        sql.Column('data', sql.Text()))
    user_sid_metadata_table.create(migrate_engine, checkfirst=True)
...
def downgrade(migrate_engine):
    ...
    tables = ['user_sid_metadata', 'credential', 'sid']
    for t in tables:
